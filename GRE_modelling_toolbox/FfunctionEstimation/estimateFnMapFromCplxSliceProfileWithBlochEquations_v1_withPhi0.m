function [ Fn_mat] = estimateFnMapFromCplxSliceProfileWithBlochEquations_v1_withPhi0(pulse, Gsus, Gphi0, vx_size,  mask, te, bSliceScaling, B1_map)
...
%   @param   Gphi0           Gradient map of phi0 in z direction [Ny, Nx, Nz]
...
%   November 2019; Last revision: 01-November-2019
    ...
    parfor vx=1:Nvx
        ...
        phi0 = Gphi0_vx.*z;
        phi0_mat = repmat(phi0, [Nte + 1, 1]);
        
        %dephasing weighted with the profile and integrate
        Mxy_te = Mxy_mat.*exp(1i*wz_mat.*te_mat*1E-3 + 1i.*theta_mat  + 1i.*phi0_mat);
