function [ Fn_mat] = estimateFnMapFromCplxSliceProfileWithBlochEqnAndZShimming_v1(pulse, Gsus, MzShim, vx_size,  mask, te, bSliceScaling, B1_map)
%ESTIMATEFN Estimates the dephasing of spins by taking into account the
%slice profile, which is calculated by numerical integration of the Bloch 
%equations, assuming a constant field gradient Gsus pluse a slice and echo 
%specific z-shim moments. 
%   @param   Gsus            Gradient map of B0 in z direction in mT/m [Ny, Nx, Nz]
%   @param   MzShim          zShim moments between echoes Nslices, Nte]
%                            unit is in mT/m µs (Siemens convention)                             
%   @param   vx_size         Voxel size in z in mm 
%   @param   opts_zShim      Struct containing options for zShimming
...
%        martin.soellradl@medunigraz.at
%         
% LAST MODIFIED: Septemeber 2020
        ...
        if isfield(pulse, 'GsPolarity')
            switch pulse.GsPolarity
                ...
                case 'negative'
                    polarity = -1;  
                otherwise
                    warning('pulse.GsPolarity can either be ''positive'' or ''negativ''!');
            end
        else
    ...
    MzShim_pad = [zeros(size(MzShim,1),1), MzShim]; 
    for vx=1:Nvx
        if ~mod(vx,5000)
        ...
        z_mat = repmat(z, [Nte+1, 1]); 
        MzShim_mat = repmat(MzShim_pad(idz(vx), :)', [1,Nq]);
        phi_ith_echo = gamma*MzShim_mat.*1E-6.*z_mat; %rad
        phi_shim = cumsum(phi_ith_echo); 
        ...
        Mxy_te = Mxy_mat.*exp(1i*wz_mat.*te_mat*1E-3 + 1i.*theta_mat).*exp(1i*phi_shim);
