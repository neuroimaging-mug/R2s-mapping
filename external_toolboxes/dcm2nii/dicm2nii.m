...
% %MS hack: crop string if it is longer than namelengthmax
% idx = find(cellfun(@numel, rNames)>namelengthmax)
% for i=1:length(idx); 
%    tmp = rNames{idx(i)}
%    l_string = lengtht(tmp); 
%    diff = l_string - namelengthmax; 
% end
...
%MS hack: We want to have all echoes 
if isfield(s, 'NumberOfTemporalPositions'); 
    for i=0:s.NumberOfTemporalPositions-1
       s.EchoTime(i+1) = asc_header(s, ['alTE[', num2str(i), ']'])/1000;
    end
end
