function [output,fitting,analysis] = MERA_with_Fm(data,fitting,Fn_vec, analysis)
...
%   Prasloski, T., M�dler, B., Xiang, Q.-S., MacKay, A., & Jones, C. (2012)
...
%
% Venkataramanan, L., Yi-Qiao Song, & H�rlimann, M. D. (2002).
  ...
  function [Amat] = buildA(theta_vector)
   ...
   % 2D problems are not compatible with B1fitting, so theta_vector = 180
    ...
    for k = 1:numbertheta
      ...
      if th == 180
        A(:,1:(K1-1)) = repmat(fitting.Fn,[1, fitting.numberT]).*exp(-t*(1./fitting.T'));
      else
        for kt = 1:(K1-1)
...
acceptablefittingfields = {'regtyp','regadj','numbergauss','widthgauss', ...
  ...
  'numberechoes2','H','theta','T0gauss','numberT0','numbertrains','fixedT1', 'Fn'};
defaultfittingvalue = {'mc','gcv',2,10,1e-1,1,[],100,[],[130 180],10, ...
  'n','nnlsmex','n',[],35,[],[],[],[],[],[],[],[],1,[],1,1};
...
fitting = struct('regtyp',regtyp,'regadj',regadj,'numbergauss',numbergauss, ...
  ...
  'numbertrains',numbertrains,'fixedT1',fixedT1,'Fn', Fn);
