function [P, ranges] = interleavedPattern_CAIPI(data, AF, ACL, symmetric, PhaseEncDir)
...
refRangeSlice = (dimSlice/2 - ACL/2) + 1 : (dimSlice/2 + ACL/2);
minSlice = dimSlice/2-ACL/2 + 1;
maxSlice = dimSlice/2+ACL/2;
k = dimPhase/dimSlice;
AF_phase = round(k*sqrt(AF/k));
AF_slice = round(sqrt(AF/k));

for n = 1 : noEchoes
    startSlice = start;
    for iSlice = 1:AF_slice:dimSlice
        
        % derive range of undersampling pattern
        if symmetric
            AFRange1 = startSlice:AF_phase:dimPhase/2;
            AFRange2 = dimPhase - (startSlice - 1) : -AF_phase : dimPhase/2;
            ...
            AFRange = startSlice:AF_phase:dimPhase;
            if iSlice > minSlice && iSlice < maxSlice
        ...
        switch PhaseEncDir
            case 'COL'
                ...
                P(refRange, : , refRangeSlice, n, :) = data(refRange, : , refRangeSlice, n, :);
            case 'ROW'
                P(:, range, iSlice, n, :) = data(:, range, iSlice, n, :);
                P(:, refRange, refRangeSlice, n, :) = data(:, refRange, refRangeSlice, n, :);
        end
        if startSlice == start 
            startSlice = round(AF_phase/2)+start;
        else
            startSlice = start;
        end
    end
    %     P(range, :, n, :) = data(range, :, n, :);
    start = round(AF_phase/4)+start;
    ranges{n} = range;
end
